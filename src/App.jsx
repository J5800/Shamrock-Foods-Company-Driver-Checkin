...
const DriverCheckIn = () => {
  ...
  const handleSubmit = async () => {
    ...
    // notification
    setNotifications(n => [
      { id: Date.now(), time, message: `${formData.driverName} checked in at ${formData.assignedDock}` },
      ...n
    ])
  ...
  const handleCheckOut = (checkInId) => {
    ...
    if (checkIn) {
      ...
      setCheckIns(ci =>
        ci.map(c =>
          c.id === checkInId
            ? { ...c, status: 'Checked Out', checkOutTime }
            : c
        )
      )
      ...
      // Add notification
      setNotifications(n => [
        { id: Date.now(), time: checkOutTime, message: `${checkIn.driverName} checked out from ${checkIn.dock}` },
        ...n
      ])
    }
  }
